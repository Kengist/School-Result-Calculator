...
const Registerrlt = () => {
  ...
  const savedata = (e) => {
    setAllGrade({
      Mathematics: [
        parseInt(subjects.Mathematics),
        parseInt(subjectsEx.Mathematics),
      ],
  ...
  return (
    <>
      {data && (
        <motion.div
          className="sidebar h-sreen bg-purple-50 w-3/4"
        ...
        >
          ...
          <button
            type="button"
            className="bg-purple-500 text-white p-4 shadow-sm mx-auto mt-3 block"
            onClick={show}
          >
            Get all Students
          </button>
          ...
          <p className="text-red-600  font-bold text-center">
            Click First to Get All Student details
          </p>

          {message && (
            <span className="ml-24 bg-green-200 text-green-500 my-4 block p-2 w-3/4">
          ...
          <form onSubmit={handleSubmit}>
                ...
                return (
                  ...
                  >
                    ...
                    <div className=" m-10">
                      ...
                      >
                        {cae.map((ca) => {
                          return <option key={ca}>{ca}</option>;
                        })}
                      </select>
                      ...
                      >
                        {exame.map((ex) => {
                          return <option key={ex}>{ex}</option>;
                        })}
                      </select>
                    </div>
                  </div>
            ...
            <div className="flexflex-cols ml-24 w-3/4">
              <button
                type="button"
                onClick={savedata}
                className="bg-gray-600 text-purple-200 block w-full mt-4 p-4 rounded-sm text-xl hover:bg-gray-800"
              >
                Save Records
              </button>
              <input
                type={`submit`}
                className="bg-purple-600 text-purple-200 block w-full mt-4 p-4 rounded-sm text-xl hover:bg-purple-800"
              />
            </div>
          </form>
        </motion.div>
