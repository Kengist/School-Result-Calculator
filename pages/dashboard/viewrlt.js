  ...
  return (
    <>
      ...
      >
        ...
        <button
          className="mx-auto block rounded bg-gray-500 p-4 w-1/2 my-2 text-white"
          onClick={getStudents}
        >
          Load data
        </button>
        {theScore.length ? (
          ...
          <p className="text-red-600  font-bold text-center">
            Click Load Data to Display All Students
          </p>
        )}
              ...
              theScore.map((data, index) => {
                return (
                  <tr className="hover:bg-purple-200" key={index}>
                    ...
                    <td>{calculateScore(data.studentid)}</td>
                    <td>
                      {calculateGrade(data.studentid)}
                      <span
                        className={`p-2 m-3 bg-${
                          calculateGrade(data.studentid) < 35 ? "red" : "purple"
                        }-300`}
                      >
                        {giveGrade(calculateGrade(data.studentid))}
                      </span>
                    </td>
                    <td>
                      <Link
                        href={`/dashboard/${data.studentid}`}
                        className="border px-5 py-2 rounded-sm bg-purple-100"
                      >
                        Checkout
                      </Link>
                    </td>
